...
>
> **Used by:** `evidence-extraction` skill (Sub-task 2.2 — Inferred domain detection)
>
> **Rules:**
> - This table sets `Regulatory Context = Inferred` only — it never sets `Explicit`
...
| Domain                    | Regulatory Context | Basis for Inferred Classification |
| ------------------------- | ------------------ | ----------------------------------- |
| Banking                   | Inferred           | Heavily regulated sector (capital adequacy, data governance, consumer protection) |
| Payments                  | Inferred           | PCI DSS scope likely; financial data governance rules apply |
| Healthcare                | Inferred           | Patient data obligations, clinical systems governance, data protection requirements |
| Insurance                 | Inferred           | Financial services regulation, solvency and data governance obligations |
| Telecommunications        | Inferred           | Consumer data and communications regulation in most jurisdictions |
| Energy Infrastructure     | Inferred           | Critical national infrastructure — operational and data protection obligations |
| Government                | Inferred           | Data protection legislation, public sector governance requirements |
...
2. If a domain term is found, record `Client Domain: [Domain]` in `claude-memory/memory.md`
3. If the domain maps to `Inferred` in the table above and no explicit regulation has already been detected, set `Regulatory Context = Inferred`
4. If no domain term is found, set `Regulatory Context = Unknown`
5. Never set `Regulatory Context = Inferred` based on this table if `Regulatory Context = Explicit` has already been set from a named regulation in the artifact

---

## Domain × Geography → Regulatory Framework Inference

When **both** `Client Domain` and client geography are identifiable from artifacts, use this table to infer specific regulatory frameworks. Write results to `claude-memory/memory.md` under `## Regulatory Context (Inferred)`.
...
**`## Regulatory Context (Inferred)` write format** (add to `claude-memory/memory.md` after domain × geography match):
```
...
4. If a match is found, write to `claude-memory/memory.md` `## Regulatory Context (Inferred)` using the format above
5. If no geography is determinable, use the basic domain-level `Regulatory Context` column only (Inferred/Unknown)
