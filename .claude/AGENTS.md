...
3. Load `notes.md` — check for in-progress analysis or context from earlier stages

**Scope constraint:** `memory.md` (item 1) is required for all agents. Items 2–3 (`artifacts.md`, `notes.md`) are loaded only if they appear in the agent's row in the Agent Context Scope table. Agents must not load files outside their defined scope.

**Note on `insights.md`:** This file is loaded by the Conductor during Stage 2 — Memory Initialization and its relevant patterns are surfaced into `notes.md` for downstream agents. Individual agents do not load `insights.md` directly — access is governed by the Agent Context Scope table, and no agent requires it as a direct input.
