...
Evidence Extraction is a **mandatory system stage** (Stage 1 in the execution workflow). It is not optional and runs before any agent-level analysis begins. Full extraction instructions, contradiction handling, prior-finding cross-check, and missing evidence rules are defined in `.claude/skills/evidence-extraction/SKILL.md` — load that skill at Stage 1. The canonical finding format, value claim trace block, confidence/binding guidance, and missing evidence block format are in `.claude/references/finding-schema.md`.
...
|---|---|---|
| Conductor (Stages 0–3) | `claude-memory/memory.md`, `claude-memory/artifacts.md`, `claude-memory/insights.md`, `claude-memory/notes.md`, `plan.md`, `.claude/references/stage-workflow.md`, `.claude/references/hitl-protocol.md` | `evidence-extraction` |
...
Full stage procedures, detailed inputs/outputs, activation conditions, per-stage governance rules, stage re-entry procedures, and the post-output evidence re-entry protocol are defined in `.claude/references/stage-workflow.md`.

**Load rule:** The Conductor loads `stage-workflow.md` at Stage 0. Specialist agents (Test Architect, QA Manager, Project Manager, Client Evaluator) do not load `stage-workflow.md` — their scope is defined in the Agent Context Scope table above and their own agent files.
...
The conductor manages Stages 0–3 and oversees Stage 7 pre-processing, Stage 8 coordination, and Stage 9 quality gate sequencing. Full conductor responsibilities are defined in `agents/conductor.md`.

### Workflow Override Rules

**This workflow is a recommendation, not a lock.** User instructions override stage sequencing and agent routing. They may NOT override: (1) anti-hallucination and fabrication prevention rules, (2) mandatory sourcing rules, (3) the quality gate for client-facing outputs (if skipped, output must carry: `[NOTE: Quality gate not applied — treat as working draft only.]`).

**Precedence:** Guardrails (`copilot-instructions.md`) > Quality gate > Workflow sequencing.
...
## Stage Re-Entry and Post-Output Evidence Re-Entry

Procedures for re-entering a completed stage (including cascade re-runs and new evidence integration) and for integrating new evidence after Stage 9 output has been produced are defined in `stage-workflow.md`:
- `stage-workflow.md — ## Stage Re-Entry` — mid-workflow re-entry, cascade rules, re-entry constraints
- `stage-workflow.md — ## Post-Output Evidence Re-Entry Protocol` — materiality threshold, impact analysis, selective stage re-run rules, phase tagging rule, cross-reference update rule, scope-change propagation rule
